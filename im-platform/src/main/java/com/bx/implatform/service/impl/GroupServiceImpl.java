...
public class GroupServiceImpl extends ServiceImpl<GroupMapper, Group> implements GroupService {
    ...
    @Override
    public GroupVO adminCreateGroup(GroupVO vo) {
        UserSession session = SessionContext.getSession();
        User user = userService.getById(session.getUserId());
        // 保存群组数据
        Group group = BeanUtils.copyProperties(vo, Group.class);
    ...
    public void deleteGroup(Long groupId) {
        ...
        if (!group.getOwnerId().equals(session.getUserId())&& session.getType()!=0) {
    ...
    @Override
    public void setOwner(Long groupId, Long userId) {
        // 1. 检查群成员是否存在且未退出
        GroupMember member = groupMemberService.findByGroupAndUserId(groupId, userId);
        if (member == null || member.getQuit()) {
            throw new GlobalException("该用户不是群成员");
        }

        // 2. 检查是否已经是群主
        if (member.getType() == 0) {
            throw new GlobalException("该成员已经是群主");
        }
        //  更新群的群主ID
        LambdaUpdateWrapper<Group> groupWrapper = Wrappers.lambdaUpdate();
        groupWrapper.eq(Group::getId, groupId)
                .set(Group::getOwnerId, userId);
        this.update(groupWrapper);

        // 3. 先将原群主设置为普通成员
        LambdaUpdateWrapper<GroupMember> oldOwnerWrapper = Wrappers.lambdaUpdate();
        oldOwnerWrapper.eq(GroupMember::getGroupId, groupId)
                .eq(GroupMember::getType, 0)  // 查找原群主
                .set(GroupMember::getType, 2);  // 设置为普通成员
        groupMemberService.update(oldOwnerWrapper);

        LambdaUpdateWrapper<GroupMember> wrapper = Wrappers.lambdaUpdate();
        wrapper.eq(GroupMember::getGroupId, groupId)
                .eq(GroupMember::getUserId, userId)
                .set(GroupMember::getType, 0);
        groupMemberService.update(wrapper);

        // 4. 可选：添加日志
        log.info("设置群主，群id:{}，用户id:{}，操作人:{}", groupId, userId, SessionContext.getSession().getUserId());
    }
    @Override
    public void setAdmin(Long groupId, Long userId) {
        // 1. 检查群成员是否存在且未退出
        GroupMember member = groupMemberService.findByGroupAndUserId(groupId, userId);
        if (member == null || member.getQuit()) {
            throw new GlobalException("该用户不是群成员");
        ...
        // 2. 检查是否已经是管理员
        if (member.getType() == 1) {
            throw new GlobalException("该成员已经是管理员");
        }

        // 3. 使用LambdaUpdateWrapper更新
        LambdaUpdateWrapper<GroupMember> wrapper = Wrappers.lambdaUpdate();
        wrapper.eq(GroupMember::getGroupId, groupId)
                .eq(GroupMember::getUserId, userId)
                .set(GroupMember::getType, 1);

        groupMemberService.update(wrapper);

        // 4. 可选：添加日志
        log.info("设置群管理员，群id:{}，用户id:{}，操作人:{}", groupId, userId, SessionContext.getSession().getUserId());
    }

    @Override
    public void cancelAdmin(Long groupId, Long userId) {
        // 1. 检查群成员是否存在且未退出
        GroupMember member = groupMemberService.findByGroupAndUserId(groupId, userId);
        if (member == null || member.getQuit()) {
            throw new GlobalException("该用户不是群成员");
        ...
        // 2. 检查是否已经是管理员
        if (member.getType() != 1) {
            throw new GlobalException("该成员已经不是管理员");
        }
        ...
        wrapper.eq(GroupMember::getGroupId, groupId)
                .eq(GroupMember::getUserId, userId)
                .set(GroupMember::getType, 2);

        groupMemberService.update(wrapper);

        // 4. 可选：添加日志
        log.info("取消群管理员，群id:{}，用户id:{}，操作人:{}", groupId, userId, SessionContext.getSession().getUserId());
    }
    ...
    public List<Group> findAllGroups(){
        Integer GROUP_NOT_DISSOLVED = 0;
        return this.lambdaQuery()
                .eq(Group::getDissolve, GROUP_NOT_DISSOLVED)
                .list();
    }
    @Override
    ...
    @Override
    public void addMember(GroupInviteVO vo) {
        UserSession session = SessionContext.getSession();
        ...
        List<User> userList = vo.getFriendIds().stream()
            .map(id -> users.stream().filter(f -> f.getId().equals(id)).findFirst().get())
            .toList();

        // 批量保存成员数据
        List<GroupMember> groupMembers = userList.stream().map(f -> {
            Optional<GroupMember> optional =
                members.stream().filter(m -> m.getUserId().equals(f.getId())).findFirst();
            ...
            groupMember.setUserNickName(f.getNickName());
    ...
    @Override
    public List<GroupMemberVO> adminFindGroupMembers(Long groupId) {
        Group group = getAndCheckById(groupId);
        // 获取未退出的群成员
        List<GroupMember> members = groupMemberService.findByGroupId(groupId).stream()
                .filter(member -> !member.getQuit())  // 过滤掉已退出的成员
                .collect(Collectors.toList());
